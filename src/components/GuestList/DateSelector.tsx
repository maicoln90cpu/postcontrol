...
export const DateSelector = ({
  ...
  const getDatePrices = (date: GuestListDate) => {
    ...
    let types = date.price_types && date.price_types.length > 0 ? date.price_types : [];
    ...
    return types.map(type => {
      ...
      if (userGender === 'feminino') {
        return {
          label: priceLabel,
          price: formatPrice(prices.female)
        };
      } else if (userGender === 'masculino') {
        return {
          label: priceLabel,
          price: formatPrice(prices.male)
        };
      } else {
        return {
          label: priceLabel,
  ...
  return <div className="space-y-4">
      <div className="space-y-3">
        ...
        <div className="space-y-3">
          ...
          if (hasAlternative) {
            return <AlternativeLinkCard key={date.id} date={date} eventName={eventName || 'Evento'} onLinkClick={handleAlternativeLinkClick} />;
          }
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 space-y-2">
                      ...
                      <div className="space-y-1">
                        {date.name && <h4 className="text-lg text-left font-semibold my-[20px]">
                            {date.name}
                          </h4>}
