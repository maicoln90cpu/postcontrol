...
import { CheckCircle2, XCircle, AlertCircle, Loader2, Bell, Clock, Activity, RefreshCw, Database, Wifi, Smartphone, ArrowLeft } from 'lucide-react';
...
export const PWADiagnosticDashboard = () => {
  ...
  const updateCheck = (id: string, updates: Partial<DiagnosticCheck>) => {
    setChecks(prev => prev.map(check => check.id === id ? {
      ...check,
      ...updates
    } : check));
  };
  const runDiagnostics = async () => {
    ...
    updateCheck('browser', {
      ...
      message: browserSupported ? 'Navegador compatível' : 'Navegador não suporta PWA',
    ...
    updateCheck('vapid', {
      ...
      message: vapidValid ? 'VAPID Key válida' : vapidConfigured ? 'VAPID Key configurada mas parece inválida' : 'VAPID Key não encontrada',
      details: vapidValid ? `Key (${vapidKeyLength} chars): ${vapidKey.substring(0, 20)}...` : vapidConfigured ? `Key muito curta (${vapidKeyLength} chars, esperado ~87)` : 'Chave não configurada no .env'
    });
    ...
    updateCheck('permission', {
      ...
      message: permission === 'granted' ? 'Permissão concedida' : permission === 'denied' ? 'Permissão negada' : 'Permissão não solicitada',
    ...
    if (user) {
      try {
        const {
          data: subscriptions,
          error
        } = await supabase.from('push_subscriptions').select('id, endpoint, created_at, last_used_at').eq('user_id', user.id);
        if (error) throw error;
        updateCheck('database', {
          status: subscriptions && subscriptions.length > 0 ? 'success' : 'warning',
          message: subscriptions && subscriptions.length > 0 ? `${subscriptions.length} subscription(s) registrada(s)` : 'Nenhuma subscription no banco',
          details: subscriptions && subscriptions.length > 0 ? `Última atualização: ${new Date(subscriptions[0].last_used_at).toLocaleString('pt-BR')}` : 'Faça login e ative as notificações'
        });
      } catch (error) {
        updateCheck('database', {
          status: 'error',
  ...
  const loadNotificationHistory = async () => {
    ...
    try {
      ...
      if (error) throw error;
      if (logs) {
        setNotificationHistory(logs.map(log => ({
          id: log.id,
  ...
  return <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="items-center justify-between flex flex-col">
        ...
        <div className="gap-2 flex flex-col">
        ...
        }} disabled={pushLoading || isRunning} variant="outline" size="lg">
            {pushLoading ? <>
                <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                Resincronizando...
              </> : <>
          ...
          <Button onClick={sendTestNotification} disabled={sendingTest || !user} variant="outline" size="lg" className="gap-2">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {criticalIssues === 0 && warnings === 0 ? <span className="text-green-500">Operacional</span> : criticalIssues > 0 ? <span className="text-red-500">Com problemas</span> : <span className="text-yellow-500">Atenção</span>}
            </div>
      ...
      <Tabs defaultValue="checks" className="space-y-4">
        ...
        <TabsContent value="checks" className="space-y-4">
          <Card>
            ...
            <CardContent className="space-y-3">
              {checks.map(check => <div key={check.id} className="border rounded-lg p-4">
                  ...
                  {check.details && <details className="mt-2">
        ...
        <TabsContent value="subscription" className="space-y-4">
          <Card>
            ...
            <CardContent>
              {subscriptionInfo ? <div className="space-y-3">
                  <div>
            ...
            <CardContent>
              <ScrollArea className="h-[400px]">
                <div className="space-y-3">
                  {notificationHistory.length > 0 ? notificationHistory.map(notif => <div key={notif.id} className="border rounded-lg p-4">
                        <div className="flex items-start justify-between mb-2">
