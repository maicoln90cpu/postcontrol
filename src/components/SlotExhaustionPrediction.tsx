...
export const SlotExhaustionPrediction = ({
  eventId,
  eventTitle
}: SlotExhaustionPredictionProps) => {
  ...
  const generatePrediction = async () => {
    try {
      ...
      const {
        data,
        error: invokeError
      } = await supabase.functions.invoke('ai-goal-prediction', {
        body: {
          eventId,
          userId: user.id
        }
  ...
  return <Card className="p-6">
      <div className="space-y-4">
        <div className="items-center justify-between flex flex-col">
          ...
          <Button onClick={generatePrediction} disabled={loading} size="sm" className="gap-2">
            <Sparkles className="h-4 w-4" />
            {loading ? 'Gerando...' : 'Gerar Previsão'}
          </Button>
        </div>

        {error && <div className="flex items-start gap-2 p-3 rounded-lg bg-destructive/10 border border-destructive/20">
            <AlertTriangle className="h-4 w-4 text-destructive mt-0.5" />
            <p className="text-sm text-destructive">{error}</p>
          </div>}

        {prediction && <div className="space-y-4 pt-4 border-t">
            {/* Resultado Principal */}
            ...
            {/* Fatores */}
            {prediction.factors.length > 0 && <div className="space-y-2">
                <h4 className="font-semibold text-sm">Fatores Considerados</h4>
                <ul className="space-y-1">
                  {prediction.factors.map((factor, idx) => <li key={idx} className="text-sm text-muted-foreground flex items-start gap-2">
            ...
            {/* Recomendações */}
            {prediction.recommendations.length > 0 && <div className="space-y-2">
                <h4 className="font-semibold text-sm">Recomendações</h4>
                <ul className="space-y-1">
                  {prediction.recommendations.map((rec, idx) => <li key={idx} className="text-sm text-muted-foreground flex items-start gap-2">
                      <span className="text-primary mt-1">✓</span>
                      <span>{rec}</span>
                    </li>)}
                </ul>
