...
export const UserPerformance = () => {
 ...
 const handleSearch = async () => { // 🆕 Adicionar async
  if (!selectedEventId) {
    toast.error("Por favor, selecione um evento antes de buscar.");
    return;
  }
  
  // 🆕 Settar loading ANTES de buscar
  setLoading(true);
  setHasSearched(true);
  
  try {
    await loadStats(); // 🆕 Adicionar await
  } catch (error) {
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4">
        ...
        <Card className="p-6">
          ...
          <Button onClick={handleSearch} disabled={!selectedEventId || loading} className="w-full md:w-auto">
    {loading ? (
                <>
